...
function graphene_settings_validator( $input ){
	...
	if (!isset($_POST['graphene_uninstall'])) {
			...
			if ( ! empty( $social_profiles ) ){				
				...
				foreach ( $social_profiles as $social_icon ){
					if ( ! empty( $social_icon['type'] ) ){
						...
						$input['social_profiles'][$ix]['title'] = $social_icon['title'];
						$social_icon['url'] = esc_url_raw( $social_icon['url'] );
						if ( empty( $social_icon['url'] ) && $social_icon['type'] != 'rss' ){
							add_settings_error( 'graphene_options', 2, sprintf( __( 'ERROR: Bad URL entered for the %s URL.'), ucfirst( $social_icon['type'] ) ) );
						} else {
				...
				$input['social_profiles'] = $graphene_defaults['social_profiles'];
			}
		...
		if (isset($_POST['graphene_display'])) {
			...
			if ( empty($input['title_font_colour']) ) $input['title_font_colour'] = $graphene_defaults['title_font_colour'];
        ...
        $input['db_version'] = $graphene_defaults['db_version'];
	return $input;
}
