...
function graphene_update_db_to_1_1(){
    ...
    if ( ! empty( $graphene_settings['custom_feed_url'] ) ) {
        $feed_url = $graphene_settings['custom_feed_url'];
        
        $graphene_settings['use_custom_rss_feed'] = true;
        $graphene_settings['custom_rss_feed_url'] = $feed_url;
        
        unset( $graphene_settings['custom_feed_url'] );
    }
    
    // Add the RSS profile if it isn't hidden
    ...
    // The social media is to complex to update, the user will have to add them again :( 
    unset( $graphene_settings['social_media'] );
